...
// Add this block to re-disable the tab upon clicking clearDataBtn
  ...
  server <- function(input, output, session) {
    ...
    output$Tb_ClusTCR_test <- DT::renderDT(escape = FALSE, options = list(autoWidth = FALSE, lengthMenu = c(2, 5, 10, 20, 50, 100), pageLength = 2, scrollX = TRUE), {
      ...
      #     "Upload the AG and BD clusTCR table e.g., AG_ClusTCR2_output.csv"
      #   )
      # )

      if (length(AG_calls)>0 & length(BD_calls)>0) {
        rbind(AG_calls, BD_calls)
      } else if (length(AG_calls)>0) {
        AG_calls
      } else if (length(BD_calls)>0) {
        BD_calls
      } else {
        df <- as.data.frame("Upload the AG and BD clusTCR table e.g., AG_ClusTCR2_output.csv")
        names(df) <- "upload"
        df
      }
    ...
    Pie_chart_Class <- reactive({
      ...
      if (input$chart_overview == "pie") {
        ...
        total_condition_function$ID_Column <- factor(total_condition_function$ID_Column,levels = input$ID_Column_factor)
