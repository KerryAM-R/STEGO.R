...
#' @return Saves all of the processed files to the respective folders in the directory_for_project
#' @export

preprocessing_10x <- function (downloadTCRex = FALSE,
...
#' Automated preprocessing for the BD Rhapsody dataset. The input includes the unfiltered contig file and the 10x like formatting.
...
preprocessing_bd_rap <- function (downloadSeurat = FALSE,
                                  downloadTCRex = FALSE,
                                  downloadClusTCR = FALSE,
                                  downloadTCR_Explore = FALSE,
                                  csv_contig_file = "csv",
                                  ...
                                  shiny_server = FALSE)
{
  ...
  for (i in 1:num) {
    ...
    if(length(files)==5) {
      message("Five files are present")
    } else {
      if(TRUE %in% grepl("Sample_Tag",files)) {
        message("Missing the Sample Tag file")
      } else {
        message("Check that all five files are present")
      }
      next
    }
    ...
    if (length(files[grepl("barcode", files)]) == 1 && length(files[grepl("Contig",files)]) == 1 && length(files[grepl("matrix", files)]) == 1) {
      message("Files are present for in ", sub_directory)
    } else if (length(files)==2) {
      message("possible already formatted mat file in ", sub_directory)
    ...
    if(file.exists(file_name_mat)) {
      ...
      if (downloadSeurat && length(files)>2) {
        ...
        barcode <- read.table(files[grepl("barcode", files)])
        features <- read.table(files[grepl("feature", files)])
        sample_tags <- read.csv(
          files[grepl("Sample_Tag", files)],
          skip = 7, header = T
        )
        mat <- Matrix::readMM(files[grepl("matrix", files)])
      }
      ...
      if(file.exists(AG_file_name_clust) && file.exists(BD_file_name_clust) && file.exists(file_path) && file.exists(file_name_mat)) {
        ...
        if (downloadTCRex || downloadClusTCR || downloadTCR_Explore) {
          TCR <- read.table(files[grepl("ontig", files)],sep = "\t",header = T)
          head(TCR)
          TCR_unfiltered <- TCR
          ...
          TCR_unfiltered_prod_junc <- subset(TCR_unfiltered_prod, TCR_unfiltered_prod$juct_issue != "other")
      ...
      if (downloadTCRex) {
        if(file.exists(file_name_tcrex)) {
          ...
          head(contigs)
          names(contigs) <- gsub("call", "gene", names(contigs))
          contigs2 <- contigs[, names(contigs) %in% c("v_gene","j_gene", "junction_aa")]
          head(contigs2)
      ...
      if (downloadClusTCR) {
        ...
        contigs2 <- contigs[names(contigs) %in% c("v_gene","junction_aa")]
        contigs2$Sample_Name <- sub_directory
        ...
        if (nrow(TRAG) > 0) {
          file_name_clust <- paste0("1_ClusTCR/", "AG_",sub_directory, "_clusTCR_10x.csv")
          TRAG
          write.csv(TRAG, file_name_clust, row.names = F)
        }
        ...
        if (nrow(IGH) > 0) {
          file_name_clust <- paste0("1_ClusTCR/", "IgH_",sub_directory, "_clusTCR_10x.csv")
          write.csv(IGH, file_name_clust, row.names = F)
        }
    ...
    if (downloadSeurat) {
      ...
      if(file.exists(file_name_mat) && file.exists(file_name_md)) {
        ...
        contigs_lim <- contigs_merge[!names(contigs_merge) %in% c("locus",names(contigs_merge[grep("seq_identified_", names(contigs_merge))]),"cell_type_experimental","pairing_type","consensus_count", "sequence_id", "duplicate_count", "germline_alignment", "reads", "length", "cdr3", "rev_comp", "complete_vdj", names(contigs_merge[grep("fwr", names(contigs_merge))]), "cdr1", "cdr2", "productive", names(contigs_merge[grep("sequence", names(contigs_merge))]), names(contigs_merge[grep("cigar", names(contigs_merge))]), names(contigs_merge[grep("support", names(contigs_merge))]), "Sample_Tag", "sum", "dominant", "putative_cell", "juct_issue", "seq_issue", "c_call"
                                                                  # "sum","dominant","putative_cell","seq_issue","juct_issue"
        )]

        if (nrow(contigs_lim[-c(grep("[*]", contigs_lim$junction_aa)), ] > 0)) {
          contigs_lim <- contigs_lim[-c(grep("[*]", contigs_lim$junction_aa)), ]
        }

        # removed undefined sequences

        contigs_lim <- subset(contigs_lim, contigs_lim$Sample_Name != "Multiplet")
        contigs_lim <- subset(contigs_lim, contigs_lim$Sample_Name != "Undetermined")
        head(contigs_lim)

        contigs_lim$v_gene <- gsub("TRAV14DV4","TRAV14/DV4",contigs_lim$v_call)
        contigs_lim$v_gene <- gsub("TRAV29DV5","TRAV29/DV5",contigs_lim$v_call)
        contigs_lim$v_gene <- gsub("TRAV23DV6","TRAV23/DV6",contigs_lim$v_call)
        contigs_lim$v_gene <- gsub("TRAV36DV7","TRAV36/DV7",contigs_lim$v_call)
        contigs_lim$v_gene <- gsub("TRAV38-2DV8","TRAV38-2/DV8",contigs_lim$v_call)
        ...
        names(contig_BD)[1:summary(name.list)[1]] <- paste(names(contig_BD[names(contig_BD) %in% name.list]), "_BD", sep = "")
        names(contig_AG)
        contig_paired <- merge(contig_AG, contig_BD, by = c("Cell_Index","seq_identified","Clonality","Sample_Name"), all = T)
        ...
        contig_paired <- contig_paired[!names(contig_paired) %in%
                                         c("d_gene_AG","d_call_AG","cell_type_experimental_AG")]

        contig_paired_only <- contig_paired

        contig_paired_only <- subset(contig_paired_only,contig_paired_only$junction_aa_AG != "None")
        contig_paired_only <- subset(contig_paired_only,contig_paired_only$junction_aa_BD != "None")
      ...
      if (downloadTCR_Explore) {
        ...
        if(file.exists(file_path)) {
          ...
          contig_paired_only$indiv.group <- contig_paired_only$Sample_Name
          contig_paired_only$group <-  names_for_group.indiv$V2
          contig_paired_only$indiv <-  names_for_group.indiv$V1
